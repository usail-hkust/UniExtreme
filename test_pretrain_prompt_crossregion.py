...
if __name__ == "__main__":
    ...
    DETECT = False
    PostFix = f"NEW_{VERSION}_CrossRegion"
    if DETECT:
        PostFix += "_detect"
    if USE_PART:
        PostFix += "_part"
    ...
    region_mask_list = [torch.zeros(530, 900).to(device) for i in range(4)]
    region_mask_list[0][:265, :450] = 1
    region_mask_list[1][265:, :450] = 1
    region_mask_list[2][:265, 450:] = 1
    region_mask_list[3][265:, 450:] = 1
    CAL_TYPE = True
    fuxi.eval()
    with torch.no_grad():
        ...
        for test_input_surface, test_input_upper_air, test_target_surface, test_target_upper_air, target_bbox, bboxes, types, stride, times, climatologys in test_bar:
            ...
            # Create masks
            ext_mask_ori = ext_mask
            for region_id, region_mask in enumerate(region_mask_list):
                ext_mask = (ext_mask_ori + region_mask.unsqueeze(0)) > 1
                ...
                accumulators[region_id]["norm"]["gen"]["mse_sum"] += mse_sum_gen_norm.detach().cpu()
                accumulators[region_id]["norm"]["gen"]["mae_sum"] += mae_sum_gen_norm.detach().cpu()
                accumulators[region_id]["norm"]["gen"]["count"] += count_gen_norm.detach().cpu()
                accumulators[region_id]["norm"]["ext"]["mse_sum"] += mse_sum_ext_norm.detach().cpu()
                accumulators[region_id]["norm"]["ext"]["mae_sum"] += mae_sum_ext_norm.detach().cpu()
                accumulators[region_id]["norm"]["ext"]["count"] += count_ext_norm.detach().cpu()
                ...
                for i in range(batch_size):
                    ...
                    accumulators[region_id]["norm"]["gen"]["acc_sum"] += acc_sum_gen_norm.detach().cpu()
                    accumulators[region_id]["norm"]["gen"]["acc_count"] += acc_count_gen_norm
                    accumulators[region_id]["norm"]["ext"]["acc_sum"] += acc_sum_ext_norm.detach().cpu()
                    accumulators[region_id]["norm"]["ext"]["acc_count"] += acc_count_ext_norm
    ...
    universal_results = {}
    for i in range(4):
        universal_results[i] = compute_final_metrics(accumulators[i])
