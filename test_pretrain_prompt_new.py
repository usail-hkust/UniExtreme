...
if __name__ == "__main__":
    ...
    DETECT = False
    PostFix = f"NEW_{VERSION}_has_type"
    # PostFix = f"NEW_{VERSION}_has_type_heyjudy"
    if DETECT:
        PostFix += "_detect"
    if USE_PART:
        PostFix += "_part"
    ...
    with torch.no_grad():
        ...
        for test_input_surface, test_input_upper_air, test_target_surface, test_target_upper_air, target_bbox, bboxes, types, stride, times, climatologys in test_bar:
            ...
            climatologys_raw = inv_trans(climatologys)
            
            mse_sum_gen_raw, mae_sum_gen_raw, count_gen_raw = calculate_pixel_metrics(
                test_outputs_raw, test_targets_raw)
            mse_sum_ext_raw, mae_sum_ext_raw, count_ext_raw = calculate_pixel_metrics(
                test_outputs_raw, test_targets_raw, ext_mask)
            
            # Update accumulators for raw data
            accumulators["raw"]["gen"]["mse_sum"] += mse_sum_gen_raw.detach().cpu()
            accumulators["raw"]["gen"]["mae_sum"] += mae_sum_gen_raw.detach().cpu()
            accumulators["raw"]["gen"]["count"] += count_gen_raw.detach().cpu()
            accumulators["raw"]["ext"]["mse_sum"] += mse_sum_ext_raw.detach().cpu()
            accumulators["raw"]["ext"]["mae_sum"] += mae_sum_ext_raw.detach().cpu()
            accumulators["raw"]["ext"]["count"] += count_ext_raw.detach().cpu()
            ...
            for i in range(batch_size):
                ...
                # Calculate temporal ACC for raw data
                acc_sum_gen_raw, acc_count_gen_raw = calculate_temporal_acc(test_outputs_raw[i], test_targets_raw[i], climatologys_raw[i])
                acc_sum_ext_raw, acc_count_ext_raw = calculate_temporal_acc(test_outputs_raw[i], test_targets_raw[i], climatologys_raw[i], ext_mask[i])
                accumulators["raw"]["gen"]["acc_sum"] += acc_sum_gen_raw.detach().cpu()
                accumulators["raw"]["gen"]["acc_count"] += acc_count_gen_raw
                accumulators["raw"]["ext"]["acc_sum"] += acc_sum_ext_raw.detach().cpu()
                accumulators["raw"]["ext"]["acc_count"] += acc_count_ext_raw
            
            # Calculate type-specific metrics if needed
            if CAL_TYPE and bboxes[0] is not None:
                for batch_idx, batch_bboxes in enumerate(bboxes):
                    ...
                    for event_idx, event_bbox in enumerate(batch_bboxes):
                        ...
                        for event_type in event_types:
                            if event_type not in type_accumulators:
                                type_accumulators[event_type] = deepcopy(type_accumulators_unit)
                            type_accumulators[event_type]["event_count"] += 1
                            
                            y_min, x_min, y_max, x_max = [eval(e) for e in event_bbox]
                            event_ext_mask = torch.zeros((1, 530, 900)).to(device)
                            event_ext_mask[:, x_min:x_max, y_min:y_max] = 1.
                            event_ext_mask = event_ext_mask.unsqueeze(1).expand(-1, 69, -1, -1).bool()
                            
                            # Normalized metrics
                            mse_sum_ext_norm, mae_sum_ext_norm, count_ext_norm = calculate_pixel_metrics(
                                test_outputs[batch_idx:batch_idx+1], 
                                test_targets[batch_idx:batch_idx+1], 
                                event_ext_mask)
                            
                            type_accumulators[event_type]["norm"]["ext"]["mse_sum"] += mse_sum_ext_norm.detach().cpu()
                            type_accumulators[event_type]["norm"]["ext"]["mae_sum"] += mae_sum_ext_norm.detach().cpu()
                            type_accumulators[event_type]["norm"]["ext"]["count"] += count_ext_norm.detach().cpu()
                            
                            # Raw metrics
                            mse_sum_ext_raw, mae_sum_ext_raw, count_ext_raw = calculate_pixel_metrics(
                                test_outputs_raw[batch_idx:batch_idx+1], 
                                test_targets_raw[batch_idx:batch_idx+1], 
                                event_ext_mask)
                            
                            type_accumulators[event_type]["raw"]["ext"]["mse_sum"] += mse_sum_ext_raw.detach().cpu()
                            type_accumulators[event_type]["raw"]["ext"]["mae_sum"] += mae_sum_ext_raw.detach().cpu()
                            type_accumulators[event_type]["raw"]["ext"]["count"] += count_ext_raw.detach().cpu()
                            
                            # ACC
                            acc_sum_ext_norm, acc_count_ext_norm = calculate_temporal_acc(test_outputs[batch_idx], test_targets[batch_idx], climatologys[batch_idx], event_ext_mask[0])
                            type_accumulators[event_type]["norm"]["ext"]["acc_sum"] += acc_sum_ext_norm.detach().cpu()
                            type_accumulators[event_type]["norm"]["ext"]["acc_count"] += acc_count_ext_norm
                            acc_sum_ext_raw, acc_count_ext_raw = calculate_temporal_acc(test_outputs_raw[batch_idx], test_targets_raw[batch_idx], climatologys_raw[batch_idx], event_ext_mask[0])
                            type_accumulators[event_type]["raw"]["ext"]["acc_sum"] += acc_sum_ext_raw.detach().cpu()
                            type_accumulators[event_type]["raw"]["ext"]["acc_count"] += acc_count_ext_raw
    ...
    def compute_type_final_metrics(accum):
        results = {}
    ...
    type_results = {}
    for event_type, accum in type_accumulators.items():
        type_results[event_type] = compute_type_final_metrics(accum)
    
    # Save results
    save_root = "test_logs"
    os.makedirs(save_root, exist_ok=True)
    with open(os.path.join(save_root, log_path), 'w', encoding='utf-8') as f:
        json.dump({"universal": universal_results, "type": type_results}, f, indent=4)