...
class FuxiEVT(nn.Module):
    ...
    def __init__(self, 
                #  params,
                 ...
                 embed_dim=512, num_groups=32, num_heads=8, window_size=12, **kwargs):
        super().__init__()
        ...
        self.gpd_loss = GPDLoss(img_size[0])

    def forward(self, x, y=None, ratio=None):
        x = x.unsqueeze(1)
        ...
        if self.training:
            ...
            evt_loss = self.gpd_loss(x, y, ratio)
            # evt_loss = checkpoint(self.gpd_loss, x, y, ratio, use_reentrant=False)
        else:
            evt_loss = None
