...
# from timm.models.swin_transformer_v2_adapter_nh import SwinTransformerV2StageAdapter

from typing import Sequence
from freq_utils import BandPassFilterRaw, BandPassFilterWin
...
class UTransformer(nn.Module):
    ...
    """
    def __init__(self, embed_dim, num_groups, input_resolution, num_heads, window_size, depth, use_adapter=False):
        super().__init__()
        ...
        if use_adapter:
            self.layer = SwinTransformerV2StageAdapter(embed_dim, embed_dim, input_resolution, depth, num_heads, window_size)
        else:
...
class FuxiFreq(nn.Module):
    ...
    def __init__(self, 
                #  params,
                 in_shape=(1, 69, 530, 900), patch_size=(1, 8, 8), in_chans=69, out_chans=69,
                 embed_dim=512, num_groups=32, num_heads=8, window_size=12, freq_mode="win", use_adapter=False, **kwargs):
        super().__init__()
        ...
        self.u_transformer = UTransformer(embed_dim, num_groups, input_resolution, num_heads, window_size, depth=48, use_adapter=use_adapter)
        ...
        if self.freq_mode == "raw":
            self.freq_filter = BandPassFilterRaw(int(img_size[1] / patch_size[1]), int(img_size[2] / patch_size[2]), embed_dim)
        elif self.freq_mode == "win":
            self.freq_filter = BandPassFilterWin(in_shape[1], win_size=(10, 10))
        else:
            raise ValueError
...
if __name__ == '__main__':
    ...
    inputs = torch.randn(1, 69, 530, 900).cuda()
    model = Fuxi()
    model.cuda()
